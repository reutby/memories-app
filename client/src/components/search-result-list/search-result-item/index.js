...
const SearchResultItem = ({ profileItem, userProfile }) => {
    ...
    return (
        ...
        <Button component={Link} to={`/${profileItem.userId}`} >
            ...
            <Button variant="contained" color="primary" onClick={addFollowingHandler}> {userProfile?.followings.find(following=>following ===profileItem.userId) ? "Unfollow" : "Follow"  }</Button>
        </div>
    )
}
